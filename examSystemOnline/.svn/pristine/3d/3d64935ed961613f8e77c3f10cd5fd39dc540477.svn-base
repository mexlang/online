				...
				<ul class="nav nav-list">
					...
					<li class="open"><a href="#" class="dropdown-toggle"> <!--<i class="menu-icon fa fa-desktop"></i>-->
						...
						<ul class="submenu">
							...
							<li class=""><a href="${pageContext.request.contextPath}/teacherChoice/findByDecidePage"> <i
									class="menu-icon fa fa-caret-right"></i> 判断题
							</a> <b class="arrow"></b></li>
							<li class=""><a href="${pageContext.request.contextPath}/teacherChoice/findByBlankPage"> <i
									class="menu-icon fa fa-caret-right"></i> 填空题
							</a> <b class="arrow"></b></li>
							<li class=""><a href="${pageContext.request.contextPath}/teacherChoice/findBySimplePage"> <i
									class="menu-icon fa fa-caret-right"></i> 简答题
							</a> <b class="arrow"></b></li>
							<li class=""><a href="${pageContext.request.contextPath}/teacherChoice/findByDesignPage"> <i
									class="menu-icon fa fa-caret-right"></i> 算法设计题
							</a> <b class="arrow"></b></li>
						</ul></li>
					<!--自动组卷-->
			...
			<div class="main-content-inner">
				...
				<div class="page-content">
					<button class="add" onclick="$('#apple').show();"
						style="background-color: #2a91d8;color: #fff;height: 40px;outline: none;">添加选择题</button>
						...
						<div class="popup_sure">
							...
							<div class="popup_content">
								<form
									...
									method="post">
									难度：<select name="questionDifficulty" id="questionsaveDifficulty">
										<option value="简单">简单</option>
										<option value="一般">一般</option>
										<option value="困难" selected>困难</option>
									</select> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 类型：章节<select
										name="questionChater" id="questionsaveChater">
									...
									<textarea cols="65" rows="3" name="questionName"
										id="questionsaveName"></textarea>
									<input type="file" multiple name="questionPhoto"
										id="questionsavePhoto"> <br /> 选项：A.<input type="text"
										name="optionA" id="saveoptionA">B.<input type="text"
										name="optionB" id="saveoptionB"><br />
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;C.<input
										type="text" name="optionC" id="saveoptionC">D.<input
										type="text" name="optionD" id="saveoptionD"> <br> <br>
									答案：
									<textarea cols="60" rows="2" name="questionAnswer"
										id="questionsaveAnswer"></textarea>
								</form>
								<br>
								<button id="submit" type="button" onclick="saveQuestion();"
...
<script type="text/javascript">
	...
	function startUserList() {
		...
		$.post("${pageContext.request.contextPath}/teacherChoice/selectByAllChoice",
						{
	...
	function saveQuestion() {
		var questionDifficulty = $("#questionsaveDifficulty").val();
		var questionChater = $("#questionsaveChater").val();
		var questionName = $("#questionsaveName").val();
		var questionPhoto = $("#questionsavePhoto").val();
		var optionA = $("#saveoptionA").val();
		var optionB = $("#saveoptionB").val();
		var optionC = $("#saveoptionC").val();
		var optionD = $("#saveoptionD").val();
		var questionAnswer = $("#questionsaveAnswer").val();
		$.post("${pageContext.request.contextPath}/teacherChoice/increasePreserve",
						{
	...
	//更新管理信息----ajax 2
	function forUpdateChoice(questionId) {
		$('#commonDataModal').modal('show');
		$.post("${pageContext.request.contextPath}/teacherChoice/findChoicebyId",
						{
	...
	function updateChioce() {
		var questionDifficulty = $("#questionUpdateDifficulty").val();
		var questionChater = $("#questionUpdateChater").val();
		var questionName = $("#questionUpdateName").val();
		var questionPhoto = $("#questionUpdatePhoto").val();
		var optionA = $("#UpdateoptionA").val();
		var optionB = $("#UpdateoptionB").val();
		var optionC = $("#UpdateoptionC").val();
		var optionD = $("#UpdateoptionD").val();
		var questionAnswer = $("#questionUpdateAnswer").val();
		console.info(questionAnswer);
		$.post("${pageContext.request.contextPath}/teacherChoice/updateChoiceSystem",
						{
							questionDifficulty : questionDifficulty,
							questionChater : questionChater,
							questionName : questionName,
	...
	//通过ajax post 删除用户管理员
	function deleteChoice(questionId){
		alert(questionId);
	  $.post("${pageContext.request.contextPath}/teacherChoice/deleteChoiceSystem",{questionId:questionId},
	     function(data){
	         if(data>=1){
	        alert("恭喜你 删除成功");
