			...
			<div class="main-content-inner">
				...
				<div class="page-content">
					<button class="add" onclick="$('#apple').show();"
						style="background-color: #2a91d8;color: #fff;height: 40px;outline: none;">添加填空题</button>
	...
	<div class="popup app-popup" id="apple" style="display: none;">
		<div class="popup_card">
			...
			<div class="popup_sure">
				<div class="popup_title">添加填空题</div>
				<div class="popup_content">
					<form
						action="${pageContext.request.contextPath}/teacherChoice/increaseOrdinary"
						method="post">
						难度：<select name="blanksaveDifficulty" id="blanksaveDifficulty">
							<option value="简单">简单</option>
							<option value="一般">一般</option>
							<option value="困难" selected>困难</option>
						</select> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 类型：章节<select
							name="questionChater" id="blanksaveChater">
						...
						</select> <br /> 题目：(填空题用“_”表示)
						<textarea cols="65" rows="3" name="questionName"
							id="blanksaveName"></textarea>
						<br /> 答案：(填空题之间用“@@”隔开)
						<textarea cols="60" rows="2" name="questionAnswer"
							id="blanksaveAnswer"></textarea>
						<br>
					</form>
					<button id="submit" type="button" onclick="saveBlank();"
...
<script type="text/javascript">
	...
	function startBlankList() {
		...
		$.post("${pageContext.request.contextPath}/teacherChoice/selectByAllBlank",
						{
	...
	function saveBlank() {
		var questionDifficulty = $("#blanksaveDifficulty").val();
		var questionChater = $("#blanksaveChater").val();
		var questionName = $("#blanksaveName").val();
		var questionPhoto = $("#blanksavePhoto").val();
		
		var questionAnswer = $("#blanksaveAnswer").val();
		var questionType = 2;
		$.post("${pageContext.request.contextPath}/teacherChoice/increaseDecide",
						{
	...
	//更新管理信息----ajax 2
	function forUpdateBlank(questionId) {
		$('#commonDataModal').modal('show');
		$.post("${pageContext.request.contextPath}/teacherChoice/findBlankbyId",
						{
	...
	function updateBlank() {
		var questionDifficulty = $("#blankUpdateDifficulty").val();
		var questionChater = $("#blankUpdateChater").val();
		var questionName = $("#blankUpdateName").val();
		var questionPhoto = $("#blankUpdatePhoto").val();
		var questionAnswer = $("#blankUpdateAnswer").val();
		var questionId = $("#blankUpdateId").val();
		$.post("${pageContext.request.contextPath}/teacherChoice/updateDecideSystem",
						{
							questionDifficulty : questionDifficulty,
							questionChater : questionChater,
							questionName : questionName,
	...
	//通过ajax post 删除用户管理员
	function deleteBlank(questionId){
		alert(questionId);
	  $.post("${pageContext.request.contextPath}/teacherChoice/deleteBlankSystem",{questionId:questionId},
	     function(data){
	         if(data>=1){
	        alert("恭喜你 删除成功");
