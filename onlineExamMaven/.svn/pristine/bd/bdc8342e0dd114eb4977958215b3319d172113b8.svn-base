					...
					<div style="margin-left: 10px;font-size: 15px;">
						<div style="width:100%;">
							<span style="margin-left: 0;">【题文】</span> <span
								...
								<button class="revise" onclick="javascript:forUpdateDesign(${list.questionId});"
									style="background-color:#20C891;border-radius: 5px;color:white;outline: none;">修改</button>
								<button class="delete" onclick="javascript:deleteDesign(${list.questionId});"
						...
						<form action="" method="get"
							...
							<label><input type="file" multiple value="${list.questionPhoto1}"></label>
						</form>
					</div>
					<div style="width:100%;">
						<span>&nbsp;&nbsp;&nbsp;&nbsp;答案：</span>
						<textarea cols="90" rows="10" name="title">${list.questionAnswer}</textarea>
						<input type="file" multiple value="${list.questionPhoto2}">
					</div>
				</div>
			</div>
		</c:forEach>
