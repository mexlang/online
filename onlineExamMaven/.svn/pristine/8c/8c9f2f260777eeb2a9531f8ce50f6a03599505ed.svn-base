	...
	<div class="popup app-popup" id="apple" style="display: none;">
		<div class="popup_card">
			...
			<div class="popup_sure">
				<div class="popup_title">添加算法设计题</div>
				<div class="popup_content">
					<form
						action="${pageContext.request.contextPath}/teacherChoice/designPreserve"
						method="post">
						难度：<select name="questionDifficulty" id="designsaveDifficulty">
							<option value="简单">简单</option>
							<option value="一般">一般</option>
							<option value="困难" selected>困难</option>
						</select> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 类型：章节<select
							name="questionChater" id="designsaveChater">
						...
						<textarea cols="65" rows="3" name="questionName"
							id="designsaveName"></textarea>
						<input type="file" multiple name="questionPhoto1"
							id="designsavePhoto1"> <br> 答案：
						<textarea cols="60" rows="2" name="questionAnswer"
							id="designsaveAnswer"></textarea>
						<input type="file" multiple name="questionPhoto2"
							id="designsavePhoto2"> <br>
					</form>
					<button id="submit" type="button" onclick="saveDesignQuestion();"
...
<script type="text/javascript">
	...
	function startDesignList() {
		//copy 复制一份 post ajax 形式
		$
				.post(
						"${pageContext.request.contextPath}/teacherChoice/selectByAllDesign",
						{
	...
	function saveDesignQuestion() {
		var questionDifficulty = $("#designsaveDifficulty").val();
		var questionChater = $("#designsaveChater").val();
		var questionName = $("#designsaveName").val();
		var questionPhoto1 = $("#designsavePhoto1").val();
		var questionPhoto2 = $("#designsavePhoto3").val();
		var questionAnswer = $("#designsaveAnswer").val();
		$
				.post(
						"${pageContext.request.contextPath}/teacherChoice/increaseDesign",
						{
							questionDifficulty : questionDifficulty,
							questionChater : questionChater,
							questionName : questionName,
							questionPhoto1 : questionPhoto1,
							questionPhoto2 : questionPhoto2,
	...
	//更新管理信息----ajax 2
	function forUpdateDesign(questionId) {
		$('#commonDataModal').modal('show');
		$
				.post(
						"${pageContext.request.contextPath}/teacherChoice/findDesignbyId",
						{
	...
	function updateDesign() {
		var questionId = $("#designUpdateId").val();
		var questionDifficulty = $("#designUpdateDifficulty").val();
		var questionChater = $("#designUpdateChater").val();
		var questionName = $("#designUpdateName").val();
		var questionPhoto1 = $("#designUpdatePhoto1").val();
		var questionPhoto2 = $("#designUpdatePhoto2").val();
		var questionAnswer = $("#designUpdateAnswer").val();
		$
				.post(
						"${pageContext.request.contextPath}/teacherChoice/updateDesignSystem",
						{
							questionId : questionId,
							questionDifficulty : questionDifficulty,
							questionChater : questionChater,
	...
	//通过ajax post 删除用户管理员
	function deleteDesign(questionId) {
		$
				.post(
						"${pageContext.request.contextPath}/teacherChoice/deleteDesignSystem",
						{
							questionId : questionId
						}, function(data) {
							if (data >= 1) {
